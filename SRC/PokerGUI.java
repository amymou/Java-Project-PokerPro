...
public class PokerGUI extends JFrame implements ActionListener {
	 ...
	 * 
	 * @param seed
	 *            if -1, a random game is played, otherwise the same game is
	 *            played, in that the deck will be shuffled the same way,
		...
		if (e.getSource() == btnNewGame) {
			...
			for (int i = 0; i < PokerModel.CARDS_IN_HAND; i++) {
				if (e.getSource() == btnCards[i]) {
					pm.replaceCard(i);
					cards[i].setIcon(new ImageIcon(pm.getCardFileName(i)));
					btnCards[i].setEnabled(false);
				}
			}
		}
	 ...
	 * 
	 * @param args
	 *            [0] optional seed used for testing which determines how the
	 *            deck is shuffled
	 */
